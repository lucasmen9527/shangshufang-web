...
const books = [
  { name: '泰安统计年鉴', path: 'taian', category: '统计年鉴' },
  ...
  <div class="min-h-screen bg-background">
    ...
    <div class="container-custom py-12 md:py-16">
      ...
      <p class="text-gray-500 mb-8">
        共 <span class="text-primary font-medium">{{ filteredBooks.length }}</span> 本电子书
        <span v-if="searchQuery">，搜索"{{ searchQuery }}"</span>
      </p>

      <!-- 书籍网格 -->
      <div v-if="filteredBooks.length" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        <div
        ...
        >
          ...
          <div class="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group-hover:-translate-y-1">
            <!-- 书籍封面 -->
            <div class="aspect-[3/4] bg-gradient-to-br from-primary to-secondary p-6 flex flex-col justify-between">
              <div>
                <span class="inline-block px-2 py-1 bg-white/20 rounded text-white/90 text-xs">
                  {{ book.category }}
                </span>
              </div>
              <div class="text-center">
                ...
                <h3 class="text-white font-bold text-sm leading-tight">{{ book.name }}</h3>
              </div>
            </div>
            <!-- 底部操作 -->
            <div class="p-4 flex items-center justify-between">
              <span class="text-gray-600 text-sm truncate">{{ book.name }}</span>
      ...
      <div v-else class="text-center py-16">
        ...
        <button @click="searchQuery = ''" class="mt-4 text-primary hover:underline">清除搜索</button>
      </div>
    </div>
  </div>
</template>
